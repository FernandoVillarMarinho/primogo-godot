...
func _build_panel() -> void:
	...
	box.custom_minimum_size = Vector2(560, 720)
	add_child(box)
	...
	col.add_child(_texture_button(TEX_SELECT, func() -> void:
		if not is_input_locked():
			level_select_requested.emit()))
	col.add_child(_texture_button(TEX_QUIT, func() -> void:
		if not is_input_locked():
			quit_requested.emit()))
	col.add_child(_texture_button(TEX_CLOSE, _on_close_pressed))
...
func _texture_button(tex: Texture2D, cb: Callable) -> TextureButton:
